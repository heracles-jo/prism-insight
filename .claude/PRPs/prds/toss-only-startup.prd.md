...
|---|-------|-------------|--------|----------|---------|----------|
| 1 | 브로커별 매매 설정 로더 | `toss_config.yaml`에 매매 설정 추가, `settings.trading_settings()` | in-progress | with 2 | - | [plan](../plans/broker-trading-settings.plan.md) |
| 2 | 진입점 지연 임포트 | 크래시하는 3곳의 KIS 임포트를 함수 내부로 | pending | with 1 | - | - |
