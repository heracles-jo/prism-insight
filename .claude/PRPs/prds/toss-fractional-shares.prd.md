...
|---|-------|-------------|--------|----------|---------|----------|
| 1 | 계약 확장 + 회귀 고정 | `normalize_checked_holding`이 `Decimal` 허용, KIS/KR 무변경 테스트 | complete | - | - | [plan](../plans/completed/fractional-holding-contract.plan.md) · [report](../reports/fractional-holding-contract-report.md) |
...
| 2 | 소수 보유 정확 인식 | 어댑터 `Decimal` 파싱, 절삭 제거, 시장별 분기 | complete | - | 1 | - |
| 3 | 소수 매도 | `quantity` 소수 MARKET 매도, 창 검사, 6자리 절삭 | complete | with 4 | 2 | - |
| 4 | 금액 기반 매수 | `orderAmount` 매수, 창 검사, 체결 수량 되읽기 | complete | with 3 | 2 | - |
| 5 | 창 밖 강등 규칙 | 정수 강등, 1주 미만 처리 확정 | complete | - | 3, 4 | - |
| 6 | 추적 DB 정합성 | 소수 포지션과 `us_stock_holdings` 모델 조정 | **deferred** | - | 5 | - |

> ⚠️ **Phase 6 보류**: `us_stock_holdings`가 행별 수량을 저장하지 않고 "1행 ≈ 1주"를 전제하는 것은 `#288`에서 **의도적으로** 내린 설계 결정이다(`prism-us/tracking/db_schema.py:1006`). 이를 바꾸는 것은 소수점 지원의 하위 작업이 아니라 US 포지션 모델 자체의 재설계이며, 피라미딩·부분매도·큐잉 로직 전반에 영향을 준다. 소수점 지원과 분리해 별도 과제로 다루는 것이 옳다고 판단해 보류한다. 그동안은 **브로커 잔고가 단일 진실**이며, 부분 매도 시 `residual_quantity`로 잔여를 보고한다. 가이드 §9에 명시했다.
| 7 | 문서 | 설정 가이드·CLAUDE.md에 소수점 제약 반영 | complete | - | 5 | - |
