...
- [ ] 토스 `investor-trading` 이 삼성전자 기준 **2 행만** 반환했다. `count` 상한을 고쳐도 조회 가능 기간이 짧을 수 있다 — 에이전트 프롬프트는 수 년치를 요구한다(`max_years`). 기간 커버리지 측정 필요
...
|------|------------|------------|
| 토스 수급 조회 기간이 짧다 (삼성전자 2 행) | **H** | Phase 2 에서 기간 커버리지부터 측정. 짧으면 네이버/KIS 소스로 보완 |
...
|---|-------|-------------|--------|----------|---------|----------|
| 1 | 수급 페이지 크기 수정 | `_PAGE` 를 엔드포인트별로 분리, 수급 100 | pending | with 2 | - | - |
